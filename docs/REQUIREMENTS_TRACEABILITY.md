...
| MNY-006 | Policy for multiple queued outgoing operations against one cached balance must be explicitly chosen and documented | assessment leaves this unspecified | `lib/sync/domain/spendable_balance_policy.dart`, `docs/ARCHITECTURE.md` | `test/sync/domain/spendable_balance_policy_test.dart` | VERIFIED | DONE |
